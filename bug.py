...
query_job = 'web'  # 查什么工作
experience = '102,101'  # 经验要求102=应届，103=一年以内，101=不限
...
query_areas = ['上城区', '余杭区', '拱墅区', '西湖区', '滨江区', '萧山', '钱塘区']
...
def login():
    ...
    while hascode:
        code = driver.find_element(By.XPATH, '//*[@id="wrap"]/div/div[2]/div[2]/div[2]/div[1]/div[2]/div/span/input')
...
def bugUrl():
    ...
    query = (Jobs.select(Jobs.url, Jobs.chated, Jobs.id_job, Jobs.job_name)
             .where(((Jobs.job_name.contains('web')) | Jobs.job_name.contains('vue') | Jobs.job_name.contains('前端') | Jobs.job_name.contains('vue3') | (Jobs.job_name.contains('Web'))) & (Jobs.active != 0) & (Jobs.chated != 0)))
    query = query.execute()
    for i in query:
        if push_count == 100:
            break
